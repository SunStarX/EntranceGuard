...
void  ShowErrMessage( uint16_t  ensure) 
{
  ...
  switch(ensure)
	{
    case  0x00:
			DisplayString(0,0,"OK",TRUE);
		break;
		
	  case  0x01:
			DisplayString(0,0,"数据包接收错误",TRUE);
		break;
		
	  case  0x02:
		  DisplayString(0,0,"指纹模块没有检测到指纹！！！！",TRUE);
		break;
		
	  case  0x03:
		  DisplayString(0,0,"录入指纹图像失败",TRUE);
		break;
		
	  case  0x04:
		  DisplayString(0,0,"指纹图像太干、太淡而生不成特征",TRUE);
		break;
	  
		case  0x05:
			DisplayString(0,0,"指纹图像太湿、太糊而生不成特征",TRUE);
		break;
		
	  case  0x06:
		  DisplayString(0,0,"指纹图像太乱而生不成特征",TRUE);
		break;
		
	  case  0x07:
		  DisplayString(0,0,"指纹图像正常，但特征点太少（或面积太小）而生不成特征",TRUE);
		break;
		
	  case  0x08:
		  DisplayString(0,0,"指纹不匹配",TRUE);
		break;
		
	  case  0x09:
		  DisplayString(0,0,"对比指纹失败，指纹库不存在此指纹！",TRUE);
		break;
		
		case  0x0a:
			DisplayString(0,0,"特征合并失败",TRUE);
		break;
		
		case  0x0b:
			DisplayString(0,0,"访问指纹库时地址序号超出指纹库范围",TRUE);
		break;
		
		case  0x10:
			DisplayString(0,0,"删除模板失败",TRUE);
		break;
		
		case  0x11:
			DisplayString(0,0,"清空指纹库失败",TRUE);
		break;	
		
		case  0x15:
			DisplayString(0,0,"缓冲区内没有有效原始图而生不成图像",TRUE);
		break;
		
		case  0x18:
			DisplayString(0,0,"读写 FLASH 出错",TRUE);
		break;
		
		case  0x19:
			DisplayString(0,0,"未定义错误",TRUE);
		break;
		
		case  0x1a:
			DisplayString(0,0,"无效寄存器号",TRUE);
		break;
		
		case  0x1b:
			DisplayString(0,0,"寄存器设定内容错误",TRUE);
		break;
		
		case  0x1c:
			DisplayString(0,0,"记事本页码指定错误",TRUE);
		break;
		
		case  0x1f:
			DisplayString(0,0,"指纹库满",TRUE);
		break;
		
		case  0x20:
			DisplayString(0,0,"地址错误",TRUE);
		break;
		
		default :
			DisplayString(0,0,"模块返回确认码有误",TRUE);
		break;	
  }
